Facter.add('coreos_cloudconfig_provision_version') do
  setcode do
    Facter::Core::Execution.exec("sed -ne '/provision.erb \$Id:/p' < cloud-config.yml | awk '{print $4}'")
  end
end

Facter.add('coreos_cloudconfig_snippet_version') do
  setcode do
    Facter::Core::Execution.exec("sed -ne '/coreos_cloudconfig.erb \$Id:/p' < cloud-config.yml | awk '{print $4}'")
  end
end
